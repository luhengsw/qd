FROM ghcr.io/luhengsw/pycurl:latest
...
RUN apk add --no-cache \
    ...
    { [ "$(getconf LONG_BIT)" = "32" ] && \
        ...
        apk add --no-cache py3-numpy-dev py3-opencv py3-pillow; } && \
    apk add --no-cache --virtual .build_deps \
        cmake make perl autoconf g++ automake \
        linux-headers libtool util-linux \
        py3-pip py3-setuptools py3-wheel python3-dev && \
